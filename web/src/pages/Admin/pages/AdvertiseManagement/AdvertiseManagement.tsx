...
const AdvertiseManagement = () => {
  ...
  useEffect(() => {
    const fetchAdvertises = async () => {
      try {
        const response = await advertiseApi.getListAdvertise()
    ...
    <div className='max-w-8xl mx-auto mt-8'>
      <div className='flex items-center justify-between px-6 mb-4'>
        ...
        <div className='flex space-x-4'>
          ...
          <NavLink to={path.adminDeletedAdvertise}>
            <svg
            ...
            {filteredAdvertises.length > 0 ? (
              filteredAdvertises.map((ad, index) => (
                <tr key={ad.id}>
                  ...
                  <td className='py-2 px-4 border-b text-center'>{calculateRemainingTime(ad.endDate, ad.run) || ''}</td>
                  <td className='py-2 px-4 border-b text-center'> {ad.click === 0 ? null : ad.click}</td>
                  <td className='py-2 px-4 border-b text-center'>
                    <NavLink to={`${path.adminDetailAdvertises.replace(':id', ad.id)}`}>
                      <button className='px-3 py-1 rounded-lg bg-[#5a6268] text-white hover:bg-[#55585b]'>View</button>
                    </NavLink>
                  </td>
                </tr>
              ))
